"""Re-extract REVE latents with the paper's preprocessing pipeline.

Per the paper (Sec 3.1.1):
  1. resample to 200 Hz
  2. band-pass 0.5-99.5 Hz
  3. z-score per channel using statistics pooled across all of a subject's sessions
  4. clip values exceeding 15 standard deviations

Then window into 4-s trial-aligned segments and forward through REVE-Base.

Output is identical in layout to extract_latents.py but goes to /media/alex/DATA1/REVE/latents_pp/.
"""
...
OUT_ROOT = Path("/media/alex/DATA1/REVE/latents_pp")
BANDPASS = (0.5, 99.5)
CLIP_SIGMA = 15.0
...
def process_subject(sid, model, pos_bank, out_dir, batch_size, overwrite):
    ...
    # Pooled per-channel z-score across all of this subject's runs
    pool = np.concatenate([runs_data[r] for r in runs_data], axis=1)  # (C, T_total)
    mu = pool.mean(axis=1, keepdims=True).astype(np.float32)
    sigma = pool.std(axis=1, keepdims=True).astype(np.float32)
    ...
    with h5py.File(tmp_path, "w") as f:
        ...
        f.attrs["zscore_mu"] = mu.squeeze(1)
        f.attrs["zscore_sigma"] = sigma.squeeze(1)
    ...
    print(f"  [done] S{sid:03d}: runs={sorted(runs_data)} trial={n_trial} "
...
def main():
    ...
    print(f"REVE loaded. preprocessing: bandpass {BANDPASS} Hz, "
          f"per-subject pooled z-score, clip ±{CLIP_SIGMA}σ")
