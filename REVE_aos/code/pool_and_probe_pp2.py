"""Pool global-zscore-preprocessed REVE tokens, then run LOSO linear probe."""
...
LAT_DIR = Path("/media/alex/DATA1/REVE/latents_pp2")
EMB_NPZ = Path("/media/alex/DATA1/REVE/reports/reve_pertrial_embeddings_pp2.npz")
OUT_TXT = Path("/media/alex/DATA1/REVE/reports/reve_linear_probe_pp2.txt")
PCA_K = 50
LR_C = 1.0
D = 512
...
def main():
    if EMB_NPZ.exists():
        d = np.load(EMB_NPZ); emb, lbl, sub = d["embeddings"], d["labels"], d["subjects"]
    else:
        ...
        emb = np.concatenate(embs); lbl = np.concatenate(lbls); sub = np.concatenate(subs)
        np.savez_compressed(EMB_NPZ, embeddings=emb, labels=lbl, subjects=sub)
        print(f"  saved {EMB_NPZ}  shape={emb.shape}", flush=True)
    ...
    for i, sid in enumerate(subjects):
        ...
        if i < 3 or (i + 1) % 20 == 0 or i == len(subjects) - 1:
            print(f"  fold {i+1:3d}/{len(subjects)}  S{sid:03d}  "
                  f"acc={accs[i]:.3f}  bal={bal_accs[i]:.3f}  "
                  f"running mean acc={accs[:i+1].mean():.3f}  "
                  f"bal={bal_accs[:i+1].mean():.3f}  "
                  f"elapsed={time.time()-t0:.0f}s", flush=True)

    msg = (
        f"\nLOSO 4-class probe on GLOBAL-zscore preprocessed REVE per-window:\n"
        f"  N folds = {len(accs)}\n"
        f"  features = top-{PCA_K} PCA of attention-pooled 512-D\n"
        ...
        f"\n  paper Table 4 REVE-B (Pool):  0.537 +/- 0.005\n"
        f"  paper Table 4 REVE-B:          0.510 +/- 0.012\n"
    )
    print(msg)
